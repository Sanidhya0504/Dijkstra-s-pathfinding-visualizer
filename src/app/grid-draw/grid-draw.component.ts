...
export class GridDrawComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //the event listeners to draw walls or change positions of start or end nodes
    ...
    while (openSet.length > 0) {
      ...
      if (openSet[lowestIndex] === end) {
        ...
        for (let i = path.length - 1; i >= 0; i--) {
          this.ctxGrid.fillStyle = '#ADD8E6';
          this.ctxGrid.lineWidth = this.lineWidth;
          this.drawNode(path[i].x, path[i].y, '#ADD8E6');
          await new Promise<void>((resolve) =>
            setTimeout(() => {
              resolve();
            }, this.animDelay)
